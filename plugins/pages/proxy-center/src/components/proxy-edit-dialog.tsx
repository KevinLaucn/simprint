...
import type { ProxyFormData, Proxy } from '../types';
...
const ProxyTypeCard: React.FC<{
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex flex-col items-center justify-center p-3 rounded-lg border-2 transition-colors duration-150 ${isSelected
    ...
    >
      <div
        className={`w-8 h-8 rounded-lg flex items-center justify-center mb-1.5 ${isSelected
...
export function ProxyEditDialog({
  ...
  const hasStoredPassword = Boolean(formData.password || proxy?.password);
  ...
  const handleTogglePassword = async () => {
    ...
    if (!formData.password && proxy?.password) {
      onFormDataChange({ ...formData, password: proxy.password });
  ...
  const handlePasteProxy = async () => {
    try {
      ...
      if (!text?.trim()) {
        toast.warning(t('dialog.edit.clipboardEmpty', { defaultValue: '剪贴板为空' }));
        return;
      }
      ...
      if (parsed.host && parsed.port) {
        ...
        toast.success(t('dialog.edit.importSuccess', { defaultValue: '导入成功' }));
      } else {
        toast.error(t('dialog.edit.parseError', { defaultValue: '无法解析代理格式' }));
      }
    } catch {
      toast.error(t('dialog.edit.clipboardError', { defaultValue: '读取剪贴板失败' }));
    }
  };
  ...
  const handleTestProxy = async (silent = false) => {
    ...
    try {
      // 处理密码：优先使用输入框中的密码，否则沿用原密码
      let passwordConfig = null;
      if (formData.password) {
        passwordConfig = { value: formData.password, encrypted: false };
      } else if (proxy?.password) {
        passwordConfig = { value: proxy.password, encrypted: false };
      }
  ...
  // 保存按钮在测试或提交中时禁用
  const canSave = canTest && !testing && !submitting;

  // 处理保存：如果还没有测试过，先静默测试获取 country 和 city
  const handleSaveClick = async () => {
    // 防止重复点击
    if (testing || submitting) {
      return;
    }
  ...
  return (
    <FormattedDialog
      ...
      header={{
        ...
        description: t('dialog.edit.description', { defaultValue: '修改代理服务器配置信息' }),
      ...
      <div className="space-y-4">
          ...
          <div className="rounded-lg border border-border/50 bg-muted/20 p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="proxy-password" className="text-xs flex items-center gap-1.5">
                    <Lock className="h-3 w-3 text-[10px] text-muted-foreground font-normal" />
                    {t('dialog.create.password')}
                    <span className="">({t('dialog.edit.passwordHint')})</span>
                  </Label>
                  <div className="relative">
                    <Input
                      ...
                      className={`h-9 text-sm pr-9 ${!showPassword ? 'text-transparent caret-transparent selection:bg-transparent' : ''}`}
                      placeholder={
                        !showPassword && hasStoredPassword
                          ? ''
                          : t('dialog.create.optional', { defaultValue: '可选' })
                      }
                    />
                    {!showPassword && hasStoredPassword && (
            ...
            <div className="space-y-2">
              ...
              {testResult && (
                <div
                  className={`flex items-center gap-2 p-2.5 rounded-lg text-xs ${testResult === 'success'
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                ...
                {t('dialog.edit.pasteFromClipboard', { defaultValue: '从剪贴板导入' })}
              </Button>
            </TooltipTrigger>
